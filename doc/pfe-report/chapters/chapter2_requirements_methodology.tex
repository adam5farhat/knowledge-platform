...
\newcommand{\backlogstart}[2]{%
  ...
  \begin{longtable}{|>{\RaggedRight\arraybackslash}p{0.085\linewidth}|>{\RaggedRight\arraybackslash}p{0.235\linewidth}|>{\Centering\arraybackslash}p{0.035\linewidth}|>{\Centering\arraybackslash}p{0.06\linewidth}|>{\RaggedRight\arraybackslash}p{0.40\linewidth}|}
  \caption{#1}\label{#2}\\
  \hline
  \textbf{ID} & \textbf{User Story} & \textbf{SP} & \textbf{Pri.} & \textbf{Acceptance Criteria} \\
  \hline
  \endfirsthead
  \multicolumn{5}{r}{\footnotesize\itshape continued from previous page}\\
  \hline
  \textbf{ID} & \textbf{User Story} & \textbf{SP} & \textbf{Pri.} & \textbf{Acceptance Criteria} \\
  \hline
  \endhead
...
The platform defines a set of functional requirements organised around
...
\hline
FR-05 &
Document metadata: tags, favourites, recents, archive &
...
\hline
US-S2-05 &
As an employee, I want favourites and recents so that I can return to frequently used documents. &
3 & Med. &
